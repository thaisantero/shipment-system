class TransportModel < ApplicationRecord
  ...
  scope :by_distance, ->(distance) { where('minimum_distance <= :distance AND maximum_distance >= :distance', distance: distance) }
  ...
  def tax_distance(distance)
    available_ranges = price_by_distances.where('start_range <= :distance AND end_range > :distance', distance: distance)
  ...
  def tax_weight(weight, distance)
    available_ranges = price_by_weights.where('start_range <= :weight AND end_range > :weight', weight: weight)
    return 0 if available_ranges.empty?
  ...
  def delivery_time(distance)
    available_ranges = delivery_time_tables.where('start_range <= :distance AND end_range > :distance', distance: distance)
    return 0 if available_ranges.empty?
