class DeliveryTimeTable < ApplicationRecord
  ...
  def start_range_equal_past_end_range
    past_end_range = DeliveryTimeTable.where(transport_model_id: transport_model_id)
  ...
  def start_range_inside_range
    delivery_time_table_of_transport_model = DeliveryTimeTable.where(transport_model_id: transport_model_id).where.not(id: id)
